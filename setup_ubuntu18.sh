#!/bin/bash
# lg_html project initialization script for Ubuntu Server 18.04
# Author: Michael BAG
# Version: 1.0
...
echo "========================================"
echo "lg_html Project Initialization for Ubuntu Server 18.04"
echo "========================================"
echo
...
if [ -f /etc/os-release ]; then
    ...
    if [[ "$VERSION_ID" != "18.04" ]]; then
        echo "WARNING: This script is designed for Ubuntu 18.04, but detected version: $VERSION_ID"
...
# Install Python 3.7+ (Ubuntu 18.04 comes with Python 3.6, need newer version)
echo
...
if [ "$(printf '%s\n' "$required_version" "$python3_version" | sort -V | head -n1)" != "$required_version" ]; then
    echo "Installing Python 3.8 from deadsnakes PPA..."
    ...
    sudo apt-get install -y python3.8-pip || {
        # If pip3.8 is not available, install via get-pip
